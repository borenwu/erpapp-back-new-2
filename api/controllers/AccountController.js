...
module.exports = {
  ...
  createSupplierPayableCr: function (req, res) {
    let company_id = req.param('company_id')
    ...
    AccountService.accountPayableCr(company_id, supplier_name, op_name, amount, maker, invoice)
      .then(account => {
        res.ok({status: 200, account: account})
      })
  },

  createSupplierPayableDr: function (req, res) {
    let company_id = req.param('company_id')
    let supplier_name = req.param('supplier_name')
    let op_name = req.param('op_name')
    let amount = req.param('amount')
    ...
    AccountService.accountPayableDr(company_id, supplier_name, op_name, amount, maker, invoice)
      .then(account=>{
        res.ok({status:200,account:account})
      })
  ...
  listAccountsBySupplier: function (req, res) {
    ...
    CheckService.checkSupplierName(companyId, supplier_name)
      .then(supplier => {
        return Account.find({op_date: {'>=': startDate, '<=': endDate}, supplier_id: supplier.id}).sort('op_date ASC').populate('supplier')
      })
      .then(accounts => {
        if (!accounts) {
          return res.ok({status: 200, msg: 'no accounts found'});
        ...
        accounts.map(a => {
          a.supplier_name = a.supplier.supplier_name
        })
        return res.ok(accounts);
      })
      .catch(err => res.serverError(err));
